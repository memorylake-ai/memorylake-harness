...
Be straight about the limitation: Codex-side sync cannot reliably exclude
individual projects — session summaries are generated by the engine after the
fact, and at sync time a summary cannot be safely attributed to one project.
Pretending otherwise would be a privacy promise this plugin cannot keep. The
options that DO work: turn Codex-side sync off entirely
(`sync_on_write: false` in `~/.claude/memorylake-plugin/config.md`), or use
per-project control on the Claude Code side, where memories are per-repo and
the `sync_deny` path-prefix list in the same config file applies.
